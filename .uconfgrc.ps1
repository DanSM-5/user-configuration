...
if (Test-Command fzf) {
  ...
  $env:FZF_ALT_C_OPTS = "
    ...
    --bind `"ctrl-t:change-prompt(CWD> )+reload(pwsh -NoLogo -NoProfile -NonInteractive -Command eza -A --follow-symlinks --color=always --only-dirs --dereference --no-quotes --oneline `$PWD)`"
    --bind 'ctrl-/:change-preview-window(down|hidden|),alt-up:preview-page-up,alt-down:preview-page-down,ctrl-s:toggle-sort'"
